...
#define DELAY12MHZ			( 5 ) // For TMR3 no prescaler
#define DELAY20MHZ			( 6 )
#define DELAY48MHZ			( 12 )
#define RF_DELAY			( DELAY12MHZ )
...
unsigned char cRFBuffer[ RFBUFFER_SIZE ];
...
void vRFCaptureIsr( void )
{
    ...
    unsigned int iCCPR = 0;
    unsigned char cTMR0H, cTMR0L;
    ...
    // Calculate the width
    cWidth = ( unsigned char )( iCCPR - iOldClock );
    iOldClock = iCCPR;

//    cRFBuffer[ cDataTransferred ] = cWidth;
//
//    ++cDataTransferred;
//
//    if( cDataTransferred == 60 )
//    {
//        cDataTransferred = 0;
//        cDataTransferred = 0;
//    }
    ...
    if( cWidth > 54 )           // Check for a phase change
    {
        xRF.bBitValue = ~xRF.bBitValue; // Invert the save bit value
        xRF.bStoreData = TRUE;     // Force a bit store
    }

    if( xRF.bStoreData )
    {
        cBitValue = xRF.bBitValue;
        vShiftRight( cRFBuffer + cRFBuffer_index, cBitValue );
        ++cDataTransferred;
