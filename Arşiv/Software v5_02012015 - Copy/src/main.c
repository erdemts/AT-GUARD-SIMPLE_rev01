...
//Personel tagi okutulmadan diÃ„Å¸er tagleri okumayacak. Bayrak olacak. bUserTagReaded. Log count 0 olunca bUserTagReaded = 0;
...
#pragma config WDTPS  = 512     //2 second weak-up
...
unsigned char cTimeOutCounter;

volatile unsigned long int ClockFreq;
...
//        LED ^= 1;
    ...
    else if( PIR1bits.TMR1IF && PIE1bits.TMR1IE ) //wake up interrupt for 1 second
    {
//        if( cWeakUpState == SM_WEAKUP_RUNNING )
//        {
//            TMR1H	= TMR1H_RUNNING;
//            TMR1L	= TMR1L_RUNNING;
//            cWeakUpCounter++;
//            if( cWeakUpCounter == 20 )
//            {
//                cWeakUpState = SM_WEAKUP_DEEP_SLEEP;
//            }
//        }
//        else if( cWeakUpState == SM_WEAKUP_DEEP_SLEEP )
//        {
//            TMR1H	= TMR1H_DEEP_SLEEP; //uyanma sÃ¼resi 0.5 saniye
//            TMR1L	= TMR1L_DEEP_SLEEP;
//        }
        
        ++cAlarmCheckCounter;
        if( cAlarmCheckCounter == 5 )
        {
            LED ^= 1;
            cAlarmCheckCounter = 0;
        }
//
//        if( cAlarmCheckCounter == ALARM_CHECK_PERIODE )
//        {
//            cAlarmCheckCounter = 0;
//            xFlags.bCheckAlarm = 1;
//        }
//
//        if( xFlags.bAlarmOn )
//        {
...
//        }
    ...
    {
        if( MachineState == SM_READ_TAG )
        {
            vRFCaptureIsr();
        }
        else if( MachineState == SM_CHECK_RF )
        {
            if( RF_DEMOD_OUT )
            {
                MachineState = SM_READ_TAG;
                cWeakUpState = SM_WEAKUP_RUNNING;
                TMR1H	= TMR1H_RUNNING;
                TMR1L	= TMR1L_RUNNING;
                cWeakUpCounter = 0;
                INTCONbits.T0IF = 0;
...
//        cWeakUpState = SM_WEAKUP_RUNNING;
//        TMR1H	= TMR1H_RUNNING; //uyanma sÃ¼resi 0.5 saniye
//        TMR1L	= TMR1L_RUNNING;
        
//        vUSBTask();
//
//
//        if( xFlags.bCheckAlarm )
//        {
//            xFlags.bCheckAlarm = 0;
//            vCheckAlarm();
//        }
//
...
//    USBInHandle = 0;
    ...
    //Initialize Timer1 For Wakeup
//    TMR1H = TMR1H_RUNNING;
//    TMR1L = TMR1L_RUNNING;
    T1CON = 0b00000000; //Timer1 1:8 prescaler, internal ocs, on
...
void ProcessIO(void)
{
    ...
    if(!HIDRxHandleBusy(USBOutHandle))
    {
        ...
        if( ReceivedDataBuffer[ cRxBufferR ] == STX ) //STX, SERIAL NO, COMMAND, DATA SIZE, DATA, CRCH, CRCL, ETX
        {
            ...
            {
                for( i = 0; i < 64 ; i++ ) //Empty usb tx buffer
                {
                ...
                switch( cCommand )
                {
                    ...
                    case cmd_READ_TAG:  //5 saniye iÃƒÂ§inde tag okutulmamÃ„Â±Ã…Å¸ ise dÃƒÂ¶ngÃƒÂ¼den ÃƒÂ§Ã„Â±kacak.
                        ...
                        do
                        {
                            ...
                            if( cReadEM4102( cCardId ) )
                            {
                                ClrWdt();             //EÃ„Å¸er kart okutulmasi uzun surerse wdt reset gelir.
                                xRF.bNewCard = 1;
                            }
