...
volatile unsigned char cSafeCounter = 0;
...
unsigned char cRFSense( unsigned char cNumBits );
...
void low_isr (void)
{
    ...
    {
        if(( RFState == SM_READ_TAG ) || ( RFState == SM_SENSE_RF ))
        {
            cWidth = TMR0L;
            TMR0L = 0;
